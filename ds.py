...
def find_b_r():
    ...
    blobs = img_draw.find_blobs([(95, 255)], area_threshold=50, x_stride=10, y_stride=10)
    if blobs:
        for blob in blobs:
            w = blob[2]
            h = blob[3]
            ...
            if 6000 < s < 120000 and 0.75 < c < 1.55:
                img_draw.draw_rectangle(blob[0:4], color=(255,255,255), thickness=2)
            #if True:
                b = 240 + (w - 100) * 0.25
                bx = blob[5]
                by = blob[6]
...
def sort_corners(pts):
    ...
    # 上面的两个点中，x 小的是左上，x 大的是右上
    top = sorted(pts[:2], key=lambda p: p[0])
    # 下面两个点中，x 小的是左下，x 大的是右下
...
def find_r_r_r():
    ...
    for rect in rects:
        ...
        x, y, w, h = rect[0:4]
        cc1_x, cc1_y = rect[4], rect[5]
        cc2_x, cc2_y = rect[6], rect[7]
        cc3_x, cc3_y = rect[8], rect[9]
        cc4_x, cc4_y = rect[10], rect[11]
        pts = [(cc1_x,cc1_y),(cc2_x,cc2_y),(cc3_x,cc3_y),(cc4_x,cc4_y)]
        ptss = sort_corners(pts)
        c1_x, c1_y = ptss[0]
        c2_x, c2_y = ptss[1]
        c3_x, c3_y = ptss[2]
        c4_x, c4_y = ptss[3]
        ...
        s = w * h
        c = w / h
        left_l = len_l(ptss[0],ptss[3])
        right_l = len_l(ptss[1],ptss[2])
        top_l = len_l(ptss[0],ptss[1])
        mod_l = len_l(ptss[2],ptss[3])

        if 6000 < s < 120000 and 0.65 < c < 1.55:
        # 画矩形框

            #img.draw_cross(cx, cy, color = (0, 0, 255), thickness=2)
            img_draw.draw_rectangle(rect[0:4], color=(0,0,0), thickness=4)

            img_draw.draw_line(c1_x, c1_y, c2_x, c2_y, color=(255, 0, 0), thickness=4)
            img_draw.draw_line(c2_x, c2_y, c3_x, c3_y, color=(255, 0, 0), thickness=4)
            img_draw.draw_line(c3_x, c3_y, c4_x, c4_y, color=(255, 0, 0), thickness=4)
            img_draw.draw_line(c4_x, c4_y, c1_x, c1_y, color=(255, 0, 0), thickness=4)

            img_draw.draw_circle(c1_x, c1_y, 20, color=(255, 255, 255), thickness=2)
            img_draw.draw_circle(c2_x, c2_y, 20, color=(255, 255, 255), thickness=2)
            img_draw.draw_circle(c3_x, c3_y, 20, color=(255, 255, 255), thickness=2)
            img_draw.draw_circle(c4_x, c4_y, 20, color=(255, 255, 255), thickness=2)

            return (cx, cy, left_l, right_l, top_l, mod_l, w, h)


def zx(cx, cy, left_l, right_l, top_l, mod_l, w, h, bx, by,w1,h1):
    d = False
    ...
    if d:
        ...
        if s > 21000:
            if left_l >= right_l:
                print("左下")
                bx = int(bx + (325 - w1) * 0.175)
            else:
                print("右下",left_l, right_l)
                bx = int(bx - (325 - w1) * 0.175)
        img_draw.draw_cross(320, b, color = (255, 255, 255), thickness=2)
        img_draw.draw_cross(bx, by, color = (0, 255, 255), thickness=2)
...
def find_e(cx,cy,w,h):
    ...
    if key_decide == 1:
        ...
        jd = 0
    img_draw.draw_ellipse(cx, cy, rx, ry, jd, color = (0,250,255), thickness=2)
...
try:
    ...
    while True:
        ...
        img_bin = img.binary([(95, 255)])#.dilate(1)
        bbb = ()
        bbb = find_b_r()
        ...
        fps_s = fps.fps()
        #print(fps_s)          #打印实时帧率
        img_draw.draw_string_advanced(0, 0, 30, f'FPS: {fps_s:.3f}', color=(255, 0, 255))
