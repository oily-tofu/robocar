...
try:
    ...
    while True:
        fps.tick()    #创建一个计时器对象，用于计算和跟踪帧率（FPS）
        ...
        gc.collect()              #把图像显示在屏幕或者IDE窗口中
        print(fps.fps())


        img_gray = img.to_grayscale()
        img_bin = img_gray.binary([(200, 255)])#.dilate(2)



#        threshold = (13, 20, -14, 2, 0, 19)  # 颜色
...
#        circles = img.find_circles(threshold=2500)
#        if circles:
#            for i, circle in enumerate(circles):
#                img.draw_circle(circle.x(), circle.y(), circle.r(), color=(40, 167, 225), thickness=3)
#                print(i)
#        else:
